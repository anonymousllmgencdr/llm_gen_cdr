...
class OuterloopDataset(Dataset):
    ...
    def preprocess(self, row_data):
        ...
        out_text = output + TEXT3
        token_ids = self.tokenizer.encode(out_text)
        input_ids.extend(token_ids)
        categorys_ids.extend([0 for _ in token_ids])
        labels.extend(token_ids)
        assert len(input_ids) == len(categorys_ids), "数据处理有问题"
        assert len(input_ids) == len(labels), "数据处理有问题"

        input_ids, attention_mask = pad_array(input_ids, self.tokenizer.pad_token_id, self.max_seq_len, "right")
        categorys_ids, _ = pad_array(categorys_ids, 0, self.max_seq_len, "right")
        labels, _ = pad_array(labels, -100, self.max_seq_len, "right")
        
        result_data = {
            "input_ids": input_ids,
            "attention_mask": attention_mask,
            "token_type_ids": categorys_ids,
            "labels": labels,
        }
        return result_data
...
if __name__ == '__main__':
    ...
    model_path = "/mmu_vcg_wjc_hdd/duyong/plms/QWen/Qwen2.5-1.5B-Instruct"
    ...
    sp_dataset = OuterloopDataset(
        data_path="/mmu_vcg_wjc_hdd/duyong/code/outerloop_sft/跨域paper/sample_train_0324.json", 
        max_seq_len=3576,
        tokenizer=tokenizer
    )
    for val in tqdm.tqdm(sp_dataset):
        print(val)
        flag = input("继续")
        if flag == "n":
            exit(0)
